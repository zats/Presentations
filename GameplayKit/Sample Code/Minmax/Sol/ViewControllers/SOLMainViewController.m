...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]) {
        ...
        [[PredicatesSetup sharedInstance] setupPredicates];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[SuggestionEngine sharedInstance] enterState:[Idle class]];
}
...
- (void)_scrollViewDidEndScrolling:(UIScrollView *)scrollView {
    if (self.pagingScrollView.subviews.count > 0) {
        [[SuggestionEngine sharedInstance] enterState:[SwitchCity class]];
        [[SuggestionEngine sharedInstance] enterState:[Idle class]];
    }
}
...
@interface SOLMainViewController (Shake)
@end

@implementation SOLMainViewController (Shake)

- (void)motionEnded:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    if (event.type == UIEventTypeMotion && event.subtype == UIEventSubtypeMotionShake) {
        id<Suggestion> suggestion = [[SuggestionEngine sharedInstance] suggest];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[suggestion title] message:nil delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
    }
}
